...
function buildHero(){
  ...
  const leftNodes = [
    {label:'Siti web', icon:'globe'},
    {label:'Database', icon:'db'},
    {label:'File Excel', icon:'sheet'},
    {label:'PDF / Documenti', icon:'doc'},
    {label:'API / Cloud', icon:'cloud'},
  ];
  const rightNodes = [
    {label:'Dashboard', icon:'dash'},
    {label:'Report', icon:'doc'},
    {label:'Avvisi', icon:'bell'},
    {label:'Integrazioni', icon:'link'},
    {label:'Previsioni', icon:'trend'},
  ];
...
/* ============================================================
   SERVIZIO GRAPHICS — 11 unique animated mini graphics
   ============================================================ */
...
const serviceGraphics = [
  // 1. Automazione workflow AI — nodes connected by animated lines
  () => `${svgOpen()}
  ...
  // 2. Automazione Excel e report — spreadsheet -> report/chart
  () => `${svgOpen()}
  ...
  // 3. Consulenza AI e progettazione soluzioni — roadmap w/ decision nodes
  () => `${svgOpen()}
  ...
  // 4. Dashboard potenziate dall’AI — dashboard w/ bars + line + kpi counter
  () => `${svgOpen()}
  ...
  // 5. Document intelligence con AI — PDF scanning w/ extracted fields
  () => `${svgOpen()}
  ...
  // 6. Analisi predittiva e forecasting — curve + projection
  () => `${svgOpen()}
  ...
  // 7. Pipeline AI cloud e deployment
  () => `${svgOpen()}
  ...
  // 8. Collaborazione AI per sanità e ricerca — medical scan
  () => `${svgOpen()}
  ...
  // 9. Estrazione e integrazione dati — sources into unified lake
  () => `${svgOpen()}
  ...
  // 10. Soluzioni AI personalizzate — modules assembling
  () => `${svgOpen()}
  ...
  // 11. AI affidabile e affidabilità dei modelli — gauge + fairness scale
  () => `${svgOpen()}
...
const servicesData = [
  { title:'Automazione workflow AI', desc:'Automatizziamo flussi di lavoro aziendali ripetitivi usando AI, Python, strumenti cloud ed elaborazione dati intelligente.', bullets:[
    'Automazione di attività d’ufficio ripetitive','Connessione dei dati da siti web, database, file Excel, PDF, API e sistemi cloud','Automazione della pulizia, validazione ed elaborazione dei dati','Creazione di avvisi e notifiche automatiche','Riduzione dell’inserimento manuale dei dati e della reportistica ripetitiva','Integrazione dei workflow tra reparti e sistemi aziendali'
  ]},
  { title:'Automazione Excel e report', desc:'Sostituiamo la reportistica manuale con sistemi automatizzati e affidabili.', bullets:[
    'Generazione automatica di Excel e PDF','Automazione di report settimanali e giornalieri','Estrazione dati da file CSV e portali web','KPI e riepiloghi automatizzati','Controlli di validazione per ridurre l’errore umano'
  ]},
  { title:'Consulenza AI e progettazione soluzioni', desc:'Aiutiamo le aziende a capire dove AI e automazione possono creare valore reale prima di investire nello sviluppo completo.', bullets:[
    'Valutazione delle opportunità di automazione','Studio di fattibilità AI','Analisi dei workflow','Valutazione della preparazione dei dati','Pianificazione dell’architettura tecnica','Pianificazione del prototipo','Progettazione della roadmap di progetto','Stima di costi e implementazione'
  ]},
  { title:'Dashboard potenziate dall’AI', desc:'Costruiamo dashboard intelligenti per insight aziendali in tempo reale.', bullets:[
    'Visualizzazioni basate su PowerBI e Python','Monitoraggio KPI per management e vendite','Monitoraggio in tempo reale','Previsione dei trend','Riepiloghi generati dall’AI','Integrazione con database'
  ]},
  { title:'Document intelligence con AI', desc:'Estraiamo, riassumiamo, classifichiamo e cerchiamo informazioni aziendali usando l’AI.', bullets:[
    'Estrazione dati da PDF e fatture','Riepilogo automatico dei report','Classificazione documentale','Sistemi di ricerca nella conoscenza interna','Estrazione automatica dei campi dai file'
  ]},
  { title:'Analisi predittiva e forecasting', desc:'Sviluppiamo modelli di machine learning per prevedere trend e migliorare le decisioni.', bullets:[
    'Previsione vendite e domanda','Previsioni di workload e KPI','Analisi dei trend su serie temporali','Modelli di supporto decisionale','Insight per l’ottimizzazione operativa'
  ]},
  { title:'Pipeline AI cloud e deployment', desc:'Costruiamo pipeline AI e dati scalabili che collegano fonti dati, modelli di machine learning, API, dashboard e sistemi aziendali.', bullets:[
    'Pipeline ML basate su cloud','Deployment di API','Inferenza batch e in tempo reale','Integrazione di pipeline dati','Deployment e monitoraggio dei modelli','Sistemi backend scalabili'
  ]},
  { title:'Collaborazione AI per sanità e ricerca', desc:'Supportiamo ricerca e sviluppo AI in collaborazione con università, ospedali e partner industriali.', bullets:[
    'Prototipi AI per imaging medico','Sviluppo di modelli di ricerca','Supporto alla generazione di dati sintetici','Valutazione di sistemi AI','Implementazione di codice di ricerca','Collaborazione con team accademici e clinici'
  ]},
  { title:'Estrazione e integrazione dati', desc:'Automatizziamo la raccolta e l’integrazione dei dati da più fonti aziendali.', bullets:[
    'Estrazione dati da siti e portali web','Automazione di siti interni','Connettività ai database','Integrazione API','Estrazione dati da Excel e CSV','Estrazione dati da PDF e documenti','Integrazione con storage cloud','Integrazione con altri sistemi'
  ]},
  { title:'Soluzioni AI personalizzate', desc:'Progettiamo e implementiamo sistemi AI su misura per problemi aziendali che gli strumenti standard non riescono a risolvere.', bullets:[
    'Modelli di machine learning','Modelli di classificazione e regressione','Analisi predittiva','Sistemi di rilevamento anomalie','Sistemi di raccomandazione','Modelli di supporto decisionale','Prototipi di computer vision','Prototipi AI per imaging medico','Sistemi di elaborazione dati potenziati dall’AI'
  ]},
  { title:'AI affidabile e affidabilità dei modelli', desc:'Aiutiamo le organizzazioni a valutare i sistemi AI in termini di equità, affidabilità, incertezza, bias e robustezza.', bullets:[
    'Valutazione dell’equità','Rilevamento dei bias','Test di robustezza','Stima dell’incertezza','Conformal prediction','Analisi dell’affidabilità del modello','Valutazione AI per domini sensibili','Consulenza per AI responsabile'
  ]},
];
...
function buildServices(){
  ...
  grid.innerHTML = servicesData.map((s,i) => `
    <article class="service-card reveal tilt">
      ...
      <div class="sc-body">
        <span class="sc-num">SERVIZIO ${String(i+1).padStart(2,'0')}</span>
        <h3>${s.title}</h3>
        ...
        <button class="sc-toggle" type="button" aria-expanded="false">
          <span class="sc-toggle-text">Vedi capacità</span>
  ...
  grid.querySelectorAll('.service-card').forEach(card => {
    ...
    btn.addEventListener('click', () => {
      ...
      txt.textContent = open ? 'Nascondi capacità' : 'Vedi capacità';
    });
...
const productGraphics = [
  // 1 Sistema di reportistica automatizzata
  () => `${svgOpen('0 0 300 110')}
  ...
  // 2 Strumento AI di estrazione documentale
  () => `${svgOpen('0 0 300 110')}
  ...
  // 4 Sistema di previsione vendite
  () => `${svgOpen('0 0 300 110')}
  ...
  // 5 Assistente di ricerca nella conoscenza interna
  () => `${svgOpen('0 0 300 110')}
  ...
  // 6 Chatbot AI per customer support
  () => `${svgOpen('0 0 300 110')}
  ...
  // 7 Strumento di pulizia e validazione dati
  () => `${svgOpen('0 0 300 110')}
  ...
  // 8 Prototipo di ricerca AI per sanità
  () => `${svgOpen('0 0 300 110')}
...
const productsData = [
  { title:'Sistema di reportistica automatizzata', problem:'I report settimanali e mensili manuali richiedono ore e introducono errori.', features:'Generazione programmata, KPI automatici, esportazione Excel/PDF, controlli di validazione.', ideal:'Team operations e finance' },
  { title:'Strumento AI di estrazione documentale', problem:'Fatture, moduli e contratti richiedono un lento inserimento manuale dei dati.', features:'Estrazione automatica dei campi, parsing di PDF/fatture, export strutturato.', ideal:'Team finance e back-office' },
  { title:'Dashboard KPI aziendale', problem:'Le metriche sono distribuite in fogli di calcolo senza una fonte unica di verità.', features:'Visual PowerBI/Python, monitoraggio in tempo reale, riepiloghi generati dall’AI.', ideal:'Dirigenti e responsabili vendite' },
  { title:'Sistema di previsione vendite', problem:'La pianificazione della domanda si basa su ipotesi invece che sui dati.', features:'Previsioni su serie temporali, analisi dei trend, output di supporto decisionale.', ideal:'Team vendite, retail e operations' },
  { title:'Assistente di ricerca nella conoscenza interna', problem:'I team perdono tempo cercando documenti interni dispersi.', features:'Ricerca potenziata dall’AI, classificazione documentale, risposte riassunte.', ideal:'Grandi basi di conoscenza interne' },
  { title:'Chatbot AI per customer support', problem:'I team di supporto gestiscono richieste ripetitive e ad alto volume.', features:'Risposte automatiche, integrazione con knowledge base, logica di escalation.', ideal:'Customer service ed e-commerce' },
  { title:'Strumento di pulizia e validazione dati', problem:'Dati incoerenti compromettono reportistica e analytics.', features:'Regole di pulizia automatizzate, controlli di validazione, segnalazione errori.', ideal:'Team che uniscono dati multi-fonte' },
  { title:'Prototipo di ricerca AI per sanità', problem:'I team di ricerca hanno bisogno di prototipi affidabili per testare ipotesi AI.', features:'Modelli per imaging medico, supporto ai dati sintetici, strumenti di valutazione.', ideal:'Ospedali, università e cliniche' },
];
...
const industriesData = [
  { name:'Piccole e medie imprese', icon:'<path d="M6 21V10l9-6 9 6v11" fill="none" stroke="currentColor" stroke-width="1.6"/><path d="M10 21v-7h8v7" fill="none" stroke="currentColor" stroke-width="1.6"/>' },
  { name:'Sanità e ricerca medica', icon:'<path d="M4 12h4l2-6 4 12 2-6h4" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>' },
  { name:'Retail ed e-commerce', icon:'<circle cx="10" cy="20" r="1.4" fill="currentColor"/><circle cx="18" cy="20" r="1.4" fill="currentColor"/><path d="M3 4h3l2.4 12.2a2 2 0 0 0 2 1.6h8.2a2 2 0 0 0 2-1.6L22 8H6" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>' },
  { name:'Immobiliare', icon:'<path d="M4 21V9l8-6 8 6v12" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/><rect x="10" y="13" width="4" height="8" fill="none" stroke="currentColor" stroke-width="1.4"/>' },
  { name:'Operations e logistica', icon:'<rect x="2" y="9" width="13" height="8" rx="1" fill="none" stroke="currentColor" stroke-width="1.6"/><path d="M15 12h4l3 3v2h-7z" fill="none" stroke="currentColor" stroke-width="1.6"/><circle cx="7" cy="19" r="1.8" fill="none" stroke="currentColor" stroke-width="1.4"/><circle cx="18" cy="19" r="1.8" fill="none" stroke="currentColor" stroke-width="1.4"/>' },
  { name:'Formazione e ricerca', icon:'<path d="M2 9l10-5 10 5-10 5z" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/><path d="M6 12v5c0 1.5 3 3 6 3s6-1.5 6-3v-5" fill="none" stroke="currentColor" stroke-width="1.6"/>' },
  { name:'Team finance e reporting', icon:'<circle cx="12" cy="12" r="9" fill="none" stroke="currentColor" stroke-width="1.6"/><path d="M12 7v10M9 9.5c0-1.4 1.3-2.5 3-2.5s3 1 3 2.2c0 3-6 1.5-6 4.3 0 1.3 1.3 2.5 3 2.5s3-1.1 3-2.5" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>' },
  { name:'Telecom e servizi tecnici', icon:'<path d="M4 20l4-8 4 4 4-9 4 13" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/><circle cx="4" cy="20" r="1.3" fill="currentColor"/>' },
];
...
/* ============================================================
   ...
   ============================================================ */
const processData = [
  { t:'Call di scoperta', d:'Comprendiamo i tuoi obiettivi e il workflow attuale.' },
  { t:'Analisi workflow e dati', d:'Mappiamo fonti dati e colli di bottiglia manuali.' },
  { t:'Progettazione soluzione', d:'Progettiamo l’architettura dell’automazione e dell’approccio AI.' },
  { t:'Sviluppo prototipo', d:'Costruiamo una proof of concept funzionante.' },
  { t:'Implementazione completa', d:'Sviluppiamo e integriamo il sistema completo.' },
  { t:'Deployment e supporto', d:'Lanciamo, monitoriamo e miglioriamo continuamente.' },
];
...
function buildOrbit(){
  ...
  let html = `
    ...
    <div class="orbit-core">AI e<br>AUTOMAZIONE</div>`;
